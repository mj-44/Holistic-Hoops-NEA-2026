...
class ConeTouchFinishing(tk.Frame):
    def __init__(self, parent, controller):
        ...
        #Creating the title for the cone touch finishing drill
        title = StyledLabel(
            main_frame,
            text = "Cone Touch Finishing Drill",
            size = 21,
            bold = True
        )
        ...
        subtitle = StyledLabel(
            main_frame,
            text = "Drill Focused finishing around a defender!",
            size = 14,
            bold = False
        )
        ...
        drillImage = Image.open("assets/cone_touch_finishing.png")
        ...
        steps = [
            "1. Blue player starts with ball at free throw line",
            "2. Red player initiates game by running to a cone then going back to defense",
            "3. Blue has 3 dribbles to try to get to the rim and score",
            "4. Get a score out of 25",
        ]
    ...
    def record_make(self):
        ...
        if self.makes + self.misses >= self.total_shots:
            messagebox.showinfo("Drill Complete", "You have completed all 25 finishes!")
            return
